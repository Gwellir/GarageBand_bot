...
from tgbot.models import MessengerBot, BotUser
...
def stats_main(request, debug=0):
    ...
    bot_list = []
    for bot in MessengerBot.objects.filter(is_debug=debug, is_active=True).exclude(bound_object__icontains='filter').select_related():
        ...
        posts_per_user_kwargs = {
            f'{related_name}__registered_posts__isnull': False
        }
        posts_per_user = BotUser.objects.\
            filter(**have_posts_kwargs, **posts_per_user_kwargs).\
